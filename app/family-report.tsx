...
export default function FamilyReportScreen() {
  ...
  useEffect(() => { if (!statusMessage) return; const timer = setTimeout(() => setStatusMessage(null), 4500); return () => clearTimeout(timer); }, [statusMessage]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [savedSelection, savedSort, savedDirection] = await Promise.all([AsyncStorage.getItem(REPORT_SELECTION_KEY), AsyncStorage.getItem(REPORT_SORT_KEY), AsyncStorage.getItem(REPORT_SORT_DIRECTION_KEY)]);
      const parsedSelection = savedSelection ? JSON.parse(savedSelection) : null;
      if (savedSort === "name" || savedSort === "entries") setSortBy(savedSort); if (savedDirection === "desc") setSortDirection("desc");
  ...
  const updateSort = (next: "name" | "entries") => { setSortBy(next); void AsyncStorage.setItem(REPORT_SORT_KEY, next); }; const toggleSortDirection = () => { const next = sortDirection === "asc" ? "desc" : "asc"; setSortDirection(next); void AsyncStorage.setItem(REPORT_SORT_DIRECTION_KEY, next); };
