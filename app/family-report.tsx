...
export default function FamilyReportScreen() {
  ...
  const visibleReportLog = useMemo(() => filterReportOperations(reportLog, logActionFilter, logPeriodFilter), [reportLog, logActionFilter, logPeriodFilter]);
  ...
  const clearLog = () => Alert.alert("مسح سجل العمليات؟", "سيتم حذف سجل إنشاء ومشاركة وتنزيل التقارير من هذا الجهاز.", [{ text: "إلغاء", style: "cancel" }, { text: "مسح السجل", style: "destructive", onPress: () => { const empty = clearReportOperations(); setReportLog(empty); void AsyncStorage.setItem(REPORT_LOG_KEY, JSON.stringify(empty)); } }]);
