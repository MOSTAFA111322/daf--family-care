...
import { appendReportOperation, buildReportOperationsCsv, clearReportOperations, createReportOperation, DEFAULT_REPORT_STATUS_DURATIONS, filterReportOperations, getReportStatusDuration, getStatusDurationLabel, getStatusExpiry, isReportStatusDuration, isStatusMessageVisible, removeReportOperation, validateReportLogCustomRange, type ReportAction, type ReportLogActionFilter, type ReportLogCustomRange, type ReportLogPeriodFilter, type ReportOperation, type ReportStatusDurations, toggleSortDirection } from "@/lib/report-ui";
...
const REPORT_STATUS_DURATION_KEYS: Record<ReportAction, string> = { create: "dafء-report-create-duration", share: "dafء-report-share-duration", download: "dafء-report-download-duration" };
...
export default function FamilyReportScreen() {
  ...
  const [statusAction, setStatusAction] = useState<ReportAction | null>(null);
  const [statusExpiresAt, setStatusExpiresAt] = useState<number | null>(null);
  const [statusDurations, setStatusDurations] = useState<ReportStatusDurations>(DEFAULT_REPORT_STATUS_DURATIONS);
  const [reportLog, setReportLog] = useState<ReportOperation[]>([]);
  const [logActionFilter, setLogActionFilter] = useState<ReportLogActionFilter>("all");
  const [logPeriodFilter, setLogPeriodFilter] = useState<ReportLogPeriodFilter>("all");
  const [customRange, setCustomRange] = useState<ReportLogCustomRange>({ from: "", to: "" });
  const [showFromPicker, setShowFromPicker] = useState(false);
  const [showToPicker, setShowToPicker] = useState(false);

  useEffect(() => { if (!statusMessage || !statusAction) return; const timer = setTimeout(() => { setStatusMessage(null); setStatusAction(null); setStatusExpiresAt(null); }, getReportStatusDuration(statusAction, statusDurations)); return () => clearTimeout(timer); }, [statusMessage, statusAction, statusDurations]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [savedSelection, savedSort, savedDirection, savedLog, savedCreateDuration, savedShareDuration, savedDownloadDuration] = await Promise.all([AsyncStorage.getItem(REPORT_SELECTION_KEY), AsyncStorage.getItem(REPORT_SORT_KEY), AsyncStorage.getItem(REPORT_SORT_DIRECTION_KEY), AsyncStorage.getItem(REPORT_LOG_KEY), AsyncStorage.getItem(REPORT_STATUS_DURATION_KEYS.create), AsyncStorage.getItem(REPORT_STATUS_DURATION_KEYS.share), AsyncStorage.getItem(REPORT_STATUS_DURATION_KEYS.download)]);
      const parsedSelection = savedSelection ? JSON.parse(savedSelection) : null;
      if (savedSort === "name" || savedSort === "entries") setSortBy(savedSort); if (savedDirection === "desc") setSortDirection("desc"); const restoredDurations = { create: Number(savedCreateDuration), share: Number(savedShareDuration), download: Number(savedDownloadDuration) }; setStatusDurations({ create: isReportStatusDuration(restoredDurations.create) ? restoredDurations.create : DEFAULT_REPORT_STATUS_DURATIONS.create, share: isReportStatusDuration(restoredDurations.share) ? restoredDurations.share : DEFAULT_REPORT_STATUS_DURATIONS.share, download: isReportStatusDuration(restoredDurations.download) ? restoredDurations.download : DEFAULT_REPORT_STATUS_DURATIONS.download }); if (savedLog) { try { const parsedLog = JSON.parse(savedLog); if (Array.isArray(parsedLog)) setReportLog(parsedLog.slice(0, 12)); } catch { /* تجاهل سجل تالف */ } }
  ...
  const customRangeError = useMemo(() => logPeriodFilter === "custom" ? validateReportLogCustomRange(customRange) : null, [customRange, logPeriodFilter]);
  const visibleReportLog = useMemo(() => filterReportOperations(reportLog, logActionFilter, logPeriodFilter, Date.now(), customRange), [reportLog, logActionFilter, logPeriodFilter, customRange]);
  ...
  const recordReportOperation = (action: ReportAction) => { const next = appendReportOperation(reportLog, createReportOperation(action)); setReportLog(next); void AsyncStorage.setItem(REPORT_LOG_KEY, JSON.stringify(next)); };
  const showStatus = (action: ReportAction, message: string) => { setStatusAction(action); setStatusMessage(message); setStatusExpiresAt(getStatusExpiry(Date.now(), getReportStatusDuration(action, statusDurations))); };
  const clearLog = () => Alert.alert("مسح سجل العمليات؟", "سيتم حذف سجل إنشاء ومشاركة وتنزيل التقارير من هذا الجهاز.", [{ text: "إلغاء", style: "cancel" }, { text: "متابعة", onPress: () => Alert.alert("تأكيد نهائي لمسح السجل", "لن يمكن استعادة العمليات المحذوفة من هذا الجهاز.", [{ text: "إلغاء", style: "cancel" }, { text: "مسح نهائي", style: "destructive", onPress: () => { const empty = clearReportOperations(); setReportLog(empty); void AsyncStorage.setItem(REPORT_LOG_KEY, JSON.stringify(empty)); } }]) }]);
  const removeLogOperation = (operationId: string) => Alert.alert("حذف العملية؟", "سيتم حذف هذه العملية من السجل المحلي.", [{ text: "إلغاء", style: "cancel" }, { text: "حذف", style: "destructive", onPress: () => { const next = removeReportOperation(reportLog, operationId); setReportLog(next); void AsyncStorage.setItem(REPORT_LOG_KEY, JSON.stringify(next)); } }]);
  const updateCustomDate = (key: "from" | "to", date: Date) => setCustomRange((current) => ({ ...current, [key]: date.toISOString().slice(0, 10) }));
  const exportLogCsv = async () => {
    const csv = buildReportOperationsCsv(visibleReportLog);
    if (Platform.OS === "web") {
      const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([csv], { type: "text/csv;charset=utf-8" })); link.download = "dafء-report-operations.csv"; link.click(); URL.revokeObjectURL(link.href); return;
    }
    const uri = `${FileSystem.cacheDirectory}dafء-report-operations.csv`;
    await FileSystem.writeAsStringAsync(uri, csv, { encoding: FileSystem.EncodingType.UTF8 });
    if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(uri, { mimeType: "text/csv", dialogTitle: "تصدير سجل العمليات" });
  };

  const createPdf = async () => {
    setBusy(true);
    try {
      const html = buildFamilyReportHtml(sortedSelectedMembers, filteredHistories, periodLabel, visibleReportLog);
  ...
  const shareReport = async () => {
    ...
    recordReportOperation("share"); showStatus("share", "تم تجهيز التقرير للمشاركة.");
  };
