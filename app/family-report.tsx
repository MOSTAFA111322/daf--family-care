...
export default function FamilyReportScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [savedSelection, savedSort] = await Promise.all([AsyncStorage.getItem(REPORT_SELECTION_KEY), AsyncStorage.getItem(REPORT_SORT_KEY)]);
      const parsedSelection = savedSelection ? JSON.parse(savedSelection) : null;
      if (savedSort === "name" || savedSort === "entries") setSortBy(savedSort);
  ...
  const sortedSelectedMembers = useMemo(() => [...selectedMembers].sort((a, b) => {
    if (sortBy === "name") return a.name.localeCompare(b.name, "ar") || a.id.localeCompare(b.id);
    return ((filteredHistories[b.id]?.length ?? 0) - (filteredHistories[a.id]?.length ?? 0)) || a.name.localeCompare(b.name, "ar") || a.id.localeCompare(b.id);
  }), [filteredHistories, selectedMembers, sortBy]);

  const updateSelection = (ids: string[]) => { setSelectedIds(ids); void AsyncStorage.setItem(REPORT_SELECTION_KEY, JSON.stringify(ids)); };
  const updateSort = (next: "name" | "entries") => { setSortBy(next); void AsyncStorage.setItem(REPORT_SORT_KEY, next); };
  ...
  const shareReport = async () => {
    ...
    setStatusMessage("تم تجهيز التقرير للمشاركة.");
  };
