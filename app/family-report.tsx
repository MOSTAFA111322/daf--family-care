...
export default function FamilyReportScreen() {
  ...
  const sortedSelectedMembers = useMemo(() => [...selectedMembers].sort((a, b) => sortBy === "name" ? a.name.localeCompare(b.name, "ar") : (filteredHistories[b.id]?.length ?? 0) - (filteredHistories[a.id]?.length ?? 0)), [filteredHistories, selectedMembers, sortBy]);
