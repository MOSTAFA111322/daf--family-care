...
const deviceUses24HourClock = deviceHourCycle === "h23" || deviceHourCycle === "h24";
const formatTime = (hour: number, minute: number, use24HourClock: boolean) => new Intl.DateTimeFormat(undefined, { hour: "numeric", minute: "2-digit", hour12: !use24HourClock }).format(new Date(2020, 0, 1, hour, minute));
...
export default function NotificationsScreen() {
  ...
  const [hourFormat, setHourFormat] = useState<"device" | "12" | "24">("device");
  const uses24HourClock = hourFormat === "24" || (hourFormat === "device" && deviceUses24HourClock);
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    const load = async () => {
      const [stored, schedule, savedFormat] = await Promise.all([AsyncStorage.getItem("dafء-weekly-summary"), getWeeklySummarySchedule(), AsyncStorage.getItem("dafء-hour-format")]);
      ...
      if (savedFormat === "12" || savedFormat === "24") setHourFormat(savedFormat);
    };
    void load();
  }, []);
