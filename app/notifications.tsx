...
import { cancelWeeklySummaryNotification, getWeeklySummarySchedule, scheduleWeeklySummaryNotification, WEEKLY_SUMMARY_DAY_KEY, WEEKLY_SUMMARY_HOUR_KEY } from "@/lib/weekly-summary-notification";
...
export default function NotificationsScreen() {
  ...
  const applySchedule = async (next: { weekday: number; hour: number }) => {
    setWeekday(next.weekday);
    setHour(next.hour);
    await AsyncStorage.multiSet([[WEEKLY_SUMMARY_DAY_KEY, String(next.weekday)], [WEEKLY_SUMMARY_HOUR_KEY, String(next.hour)]]);
    if (!enabled) return;
    setBusy(true);
    const scheduled = await scheduleWeeklySummaryNotification({ ...next, minute: 0 });
    setBusy(false);
  ...
  const toggle = async (value: boolean) => {
    setBusy(true);
    if (value) {
      const scheduled = await scheduleWeeklySummaryNotification({ weekday, hour, minute: 0 });
      if (!scheduled) {
        setBusy(false);
